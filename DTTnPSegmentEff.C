...
void DTTnPSegmentEff::book()
{
  ...
  std::cout << " ** SM: DTTnPSegmentEff::book - booking efficiency plots" << std::endl;
...
void DTTnPSegmentEff::fill(const Int_t iMu)
{
 ...
 std::cout << " ** SM: DTTnPSegmentEff::fill: about to loop over stations " << std::endl;
   ...
   for (Int_t iCh = 1; iCh < 5; ++iCh)
	   ...
	   Mu_numberOfRPCLayers_rpc->at(iMu) >= m_tnpConfig.probe_minNRPCLayers )
	{
	  ...
	  std::cout << " ** SM: DTTnPSegmentEff::fill get passing probe:  iPassingSeg = " << iPassingSeg <<  " iMu = " << iMu << " iCh = " << iCh << std::endl;
